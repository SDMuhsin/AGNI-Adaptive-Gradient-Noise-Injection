...
def parse_args():
    ...
    return args
def clean_array(arr, default_value=0):
    arr = np.array(arr)
...
def compute_with_retry(metric, max_retries=10, initial_wait=1):
    for attempt in range(max_retries):
        try:
            ...
            if hasattr(metric, '_predictions') and hasattr(metric, '_references'):
                metric._predictions = clean_array(metric._predictions)
                metric._references = clean_array(metric._references)
            ...
            if "Please specify an experiment_id" in str(e):
                ...
                # Unknown error, likely found NaNs or infs
                print(f"Error encountered: {str(e)}. Returning default values.")
                return get_default_metric_values(metric)

def get_default_metric_values(metric):
    # This function should return a dictionary with the same keys as metric.compute()
    # but with all values set to 0
    if metric.name == "glue":
        if metric.config_name == "stsb":
            return {"pearson": 0.0, "spearmanr": 0.0}
        elif metric.config_name in ["mrpc", "qqp"]:
            return {"accuracy": 0.0, "f1": 0.0, "combined_score": 0.0}
        elif metric.config_name in ["cola"]:
            return {"matthews_correlation" : 0.0}
        else:
            return {"accuracy": 0.0}
    else:
        # For other metrics, you might need to add more cases
        # or find a way to dynamically determine the expected keys
        return {"score": 0.0}
def compute_with_retry_depracated(metric, max_retries=10, initial_wait=1):
    for attempt in range(max_retries):
        try:
            return metric.compute()
        except ValueError as e:
...
def main():
    ...
    args = parse_args()
    args.job_id += f"_{args.optimizer}_lr{args.learning_rate}_e{args.num_train_epochs}"
    ...
    if(args.optimizer == 'adamw'):
        ...
        optimizer = torch.optim.ASGD(optimizer_grouped_parameters, lr = args.learning_rate)
    elif (args.optimizer == 'adadelta'):
        optimizer = torch.optim.Adadelta(optimizer_grouped_parameters, lr = args.learning_rate)
    ...
    '''
        Save file structure for tracking runtime
        coarse_time_model_task_optimizer_epoch.json
        {
            time_per_epoch : _,
            time_per_batch : _,
    ...
    epoch_counter = 1
    batch_counter = 1
    time_tracker_save_file_name = f"coarse_time_m{args.model_name_or_path.split('/')[-1]}_t{args.task_name}_o{args.optimizer}_e{args.num_train_epochs}.json"
    ...
    json.dump(tracker,open( f"./saves/{time_tracker_save_file_name}","w"))
    ...
    print(f"About to save data : ", save_data)
    json.dump(
        save_data,
        open(eval_save_file_name,'w')
    )
