...
def main():
    ...
    def update_running_statistics(mean, var, n, new_sample):
        new_sample = new_sample.to(mean.device)
        n += 1
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            ...
            # Collect gradients and update running statistics
            grad_vector = torch.cat([p.grad.view(-1) for p in model.parameters() if p.grad is not None]).detach().cpu()

            if running_mean is None:
                running_mean = torch.zeros_like(grad_vector)
                running_var = torch.zeros_like(grad_vector)

            running_mean, running_var, n_samples = update_running_statistics(running_mean, running_var, n_samples, grad_vector)

            if step % args.gradient_accumulation_steps == 0 or step == len(train_dataloader) - 1:
                # Inject variance-based noise into gradients
                if n_samples > 1:
                    var = running_var / (n_samples - 1)
                    var = var.to(model.device)

                    for p in model.parameters():
                        if p.grad is not None:
                            noise = torch.randn_like(p.grad) * torch.sqrt(eta * var[:p.grad.numel()]).view_as(p.grad)
                            p.grad += noise
