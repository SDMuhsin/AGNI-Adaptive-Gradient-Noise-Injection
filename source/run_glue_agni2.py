...
def main():
    ...
    ''' For computing effective step sizes '''
    def compute_step_size_stats(step_sizes):
        step_sizes = torch.stack(step_sizes)
        avg_step_size = torch.mean(step_sizes)
        step_size_variance = torch.var(step_sizes)
        return avg_step_size, step_size_variance
    ...
    eta = 0.01
    gradient_dict = {p: [] for p in model.parameters() if p.requires_grad}
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    
    epoch_counter = 1
    batch_counter = 1
    time_tracker_save_file_name = f"coarse_time_m{args.model_name_or_path.split('/')[-1]}_t{args.task_name}_oagni_e{args.num_train_epochs}.json"
